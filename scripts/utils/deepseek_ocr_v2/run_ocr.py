...
def load_model_transformers(gpu_id: int):
    """Load DeepSeek-OCR v2 model via transformers fallback."""
    import torch
    ...
    # DeepSeek-OCR-2 remote code imports LlamaFlashAttention2, which may be
    # absent in newer transformers versions required by vLLM.
    ...
    # DeepSeek-OCR-2 code expects DynamicCache.seen_tokens used in older
...
def infer_single_image_transformers(model, tokenizer, image_path: str, output_path: str) -> str:
    ...
    raise RuntimeError(f"DeepSeek-OCR-2 did not produce expected output file: {result_mmd}")
